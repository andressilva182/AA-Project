// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

#include "UObject/GeneratedCppIncludes.h"
#include "AA_Project/WorldGenerationManager.h"
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void EmptyLinkFunctionForGeneratedCodeWorldGenerationManager() {}
// Cross Module References
	AA_PROJECT_API UClass* Z_Construct_UClass_AWorldGenerationManager();
	AA_PROJECT_API UClass* Z_Construct_UClass_AWorldGenerationManager_NoRegister();
	ENGINE_API UClass* Z_Construct_UClass_AActor();
	UPackage* Z_Construct_UPackage__Script_AA_Project();
// End Cross Module References
	void AWorldGenerationManager::StaticRegisterNativesAWorldGenerationManager()
	{
	}
	IMPLEMENT_CLASS_NO_AUTO_REGISTRATION(AWorldGenerationManager);
	UClass* Z_Construct_UClass_AWorldGenerationManager_NoRegister()
	{
		return AWorldGenerationManager::StaticClass();
	}
	struct Z_Construct_UClass_AWorldGenerationManager_Statics
	{
		static UObject* (*const DependentSingletons[])();
#if WITH_METADATA
		static const UECodeGen_Private::FMetaDataPairParam Class_MetaDataParams[];
#endif
		static const FCppClassTypeInfoStatic StaticCppClassTypeInfo;
		static const UECodeGen_Private::FClassParams ClassParams;
	};
	UObject* (*const Z_Construct_UClass_AWorldGenerationManager_Statics::DependentSingletons[])() = {
		(UObject* (*)())Z_Construct_UClass_AActor,
		(UObject* (*)())Z_Construct_UPackage__Script_AA_Project,
	};
	static_assert(UE_ARRAY_COUNT(Z_Construct_UClass_AWorldGenerationManager_Statics::DependentSingletons) < 16);
#if WITH_METADATA
	const UECodeGen_Private::FMetaDataPairParam Z_Construct_UClass_AWorldGenerationManager_Statics::Class_MetaDataParams[] = {
		{ "IncludePath", "WorldGenerationManager.h" },
		{ "ModuleRelativePath", "WorldGenerationManager.h" },
	};
#endif
	const FCppClassTypeInfoStatic Z_Construct_UClass_AWorldGenerationManager_Statics::StaticCppClassTypeInfo = {
		TCppClassTypeTraits<AWorldGenerationManager>::IsAbstract,
	};
	const UECodeGen_Private::FClassParams Z_Construct_UClass_AWorldGenerationManager_Statics::ClassParams = {
		&AWorldGenerationManager::StaticClass,
		"Engine",
		&StaticCppClassTypeInfo,
		DependentSingletons,
		nullptr,
		nullptr,
		nullptr,
		UE_ARRAY_COUNT(DependentSingletons),
		0,
		0,
		0,
		0x009000A4u,
		METADATA_PARAMS(UE_ARRAY_COUNT(Z_Construct_UClass_AWorldGenerationManager_Statics::Class_MetaDataParams), Z_Construct_UClass_AWorldGenerationManager_Statics::Class_MetaDataParams)
	};
	UClass* Z_Construct_UClass_AWorldGenerationManager()
	{
		if (!Z_Registration_Info_UClass_AWorldGenerationManager.OuterSingleton)
		{
			UECodeGen_Private::ConstructUClass(Z_Registration_Info_UClass_AWorldGenerationManager.OuterSingleton, Z_Construct_UClass_AWorldGenerationManager_Statics::ClassParams);
		}
		return Z_Registration_Info_UClass_AWorldGenerationManager.OuterSingleton;
	}
	template<> AA_PROJECT_API UClass* StaticClass<AWorldGenerationManager>()
	{
		return AWorldGenerationManager::StaticClass();
	}
	DEFINE_VTABLE_PTR_HELPER_CTOR(AWorldGenerationManager);
	AWorldGenerationManager::~AWorldGenerationManager() {}
	struct Z_CompiledInDeferFile_FID_Users_Andres_PC_Documents_Unreal_Projects_Respositories_AA_AA_Project_AA_Project_Source_AA_Project_WorldGenerationManager_h_Statics
	{
		static const FClassRegisterCompiledInInfo ClassInfo[];
	};
	const FClassRegisterCompiledInInfo Z_CompiledInDeferFile_FID_Users_Andres_PC_Documents_Unreal_Projects_Respositories_AA_AA_Project_AA_Project_Source_AA_Project_WorldGenerationManager_h_Statics::ClassInfo[] = {
		{ Z_Construct_UClass_AWorldGenerationManager, AWorldGenerationManager::StaticClass, TEXT("AWorldGenerationManager"), &Z_Registration_Info_UClass_AWorldGenerationManager, CONSTRUCT_RELOAD_VERSION_INFO(FClassReloadVersionInfo, sizeof(AWorldGenerationManager), 3608540780U) },
	};
	static FRegisterCompiledInInfo Z_CompiledInDeferFile_FID_Users_Andres_PC_Documents_Unreal_Projects_Respositories_AA_AA_Project_AA_Project_Source_AA_Project_WorldGenerationManager_h_2623558561(TEXT("/Script/AA_Project"),
		Z_CompiledInDeferFile_FID_Users_Andres_PC_Documents_Unreal_Projects_Respositories_AA_AA_Project_AA_Project_Source_AA_Project_WorldGenerationManager_h_Statics::ClassInfo, UE_ARRAY_COUNT(Z_CompiledInDeferFile_FID_Users_Andres_PC_Documents_Unreal_Projects_Respositories_AA_AA_Project_AA_Project_Source_AA_Project_WorldGenerationManager_h_Statics::ClassInfo),
		nullptr, 0,
		nullptr, 0);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
